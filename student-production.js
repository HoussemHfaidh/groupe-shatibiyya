...
function weekDeadline(week) {
  return WeeklyClock.deadline(week, currentConfig?.settings?.weekBoundaryDay ?? (currentGroupId === "group2" ? 0 : 6));
}
...
function orderedGreenStudents(weekId) {
  ...
  return [
    ...savedOrder.filter((student) => greenStudents.includes(student)).slice().reverse(),
    ...greenStudents.filter((student) => !savedOrder.includes(student)),
  ].sort((a, b) => Number(currentConfig.readyAt?.[statusKey(b, weekId)] || 0) - Number(currentConfig.readyAt?.[statusKey(a, weekId)] || 0));
}
...
async function applyConfirmedStatus(student, weekId, status) {
  ...
  if (getFirebaseUrl()) {
    await firebaseRequest(groupPath("config"), {
      method: "PATCH",
      body: JSON.stringify({ [`statuses/${key}`]: status, [`readyAt/${key}`]: readyAt }),
    });
  } else {
    await localRequest("/api/config", {
      method: "PUT",
      body: JSON.stringify({
        ...currentConfig,
        readyAt: { ...(currentConfig.readyAt || {}), [key]: readyAt },
        statuses: {

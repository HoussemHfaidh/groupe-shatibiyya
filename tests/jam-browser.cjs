  ...
  try {
    ...
    await page.context().route('https://**/*', async route => {
      ...
      if (url.pathname.includes('loginEmails')) value = { role:'student',studentName:'أحمد',groupId:profileGroup };
        ...
        value = jam;
      } else if(url.pathname.startsWith('/review/groups/')) { if(request.method()==='PUT') review=request.postDataJSON(); value=review; } else throw new Error(`Unexpected request: ${request.url()}`);
    ...
    assert.equal(await panel.locator('.validator-option').count(),0);
    ...
    await profPanel.getByRole('button',{name:'تأكيد الطالب والآية'}).click();
    await profPanel.getByText('تم الحفظ.',{exact:true}).waitFor();
    assert.equal(jam['week-2026-09-12'].confirmations.length,1);
    assert.equal(writes,5); // Creation and first validation are a single write.
    await panel.getByRole('button',{name:'تحديث القائمة'}).click();
    await panel.getByText('أحمد — معتمد · الأستاذ',{exact:true}).waitFor();
    ...
    if(true){
      ...
      await rp.getByLabel('المدة بالدقائق').fill('12.5');
      await rp.getByLabel('عدد الأخطاء').fill('0');
      ...
      await professor.getByRole('button',{name:'القسم الثاني مكتمل — أخضر',exact:true}).click();
      await professor.getByRole('button',{name:'حفظ',exact:true}).click();
      ...
      await professor.getByRole('button',{name:'إلغاء',exact:true}).click();
      assert.equal(await grid.locator('.review-green').count(),1);

      await professor.getByRole('button',{name:'عرض حسب الطالب',exact:true}).click();
      await professor.getByLabel('تفاصيل الطالب').selectOption('علي');
      assert.match(await professor.locator('.review-details').innerText(),/12.5/);
      assert.match(await professor.locator('.review-details').innerText(),/0/);
      await professor.getByRole('button',{name:'عرض الجدول',exact:true}).click();
